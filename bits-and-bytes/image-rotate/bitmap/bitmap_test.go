...
func TestSaveBitmap(t *testing.T) {
	t.Skip()
	f, err := os.Create("test_square.bmp")
	if err != nil {
		t.Fatalf("%s", err)
	}
	defer f.Close()

	toWrite := [][]byte{squareHeader, squareInfo, squarePixels}
...
func TestNewFromReader(t *testing.T) {
	...
	if squareBmp.Offset != 54 {
		t.Errorf("Expected infoSize of %d, got %d", 54, squareBmp.InfoSize)
	}
...
func TestRotate(t *testing.T) {
	...
	if rotated.Offset != 54 {
		t.Errorf("Expected infoSize of %d, got %d", 54, rotated.InfoSize)
	}
